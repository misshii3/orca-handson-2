...
[第1弾](https://github.com/misshii3/orca-handson) を終えている前提で書いています。ワークツリーの作り方、差分の読み方、Source Control の使い方は、第1弾の該当章を示すだけにします。続編で初めて出てくる操作だけを丁寧に書きます。
...
> **スクリーンショットについて**: 画像は検証用に `orca-handson-2-test` という名前で複製したリポジトリで撮影しています。あなたの画面では `orca-handson-2` と読み替えてください。
>
...
| ワークツリー / ワークスペース（Worktree / Workspace） | 同じものです。本文は「ワークツリー」で統一しますが、Orca の画面には「ワークスペースの削除」のように両方の言い方が出ます |
| 定数ブロック | `src/price.js` 冒頭の `// ---- 定数 ----` から `// ---- 関数 ----` までの部分。#2 と #3 の両方がここに定数を足します |
...
#2 と #3 が **同じ 2 ファイル** を触ることに注目してください。どちらも `src/price.js` 冒頭の「定数ブロック」（`// ---- 定数 ----` から `// ---- 関数 ----` までの部分）に定数を足し、ファイルの末尾に関数を足します。`test/price.test.js` でも、どちらも `import` 行に関数名を足し、末尾にテストを足します。これが 9〜10 章のコンフリクトの種です。
...
Orca で、サイドバーの「プロジェクト」右側の **＋**（または「作成」メニュー → 「プロジェクトを追加」）を押します。「**フォルダーを参照**」で `~/Documents/orca-handson-2` を選びます。第1弾 5 章と同じ操作です。
...
4 つとも ✓ なら、設定（⌘,）→ 「**Agent**」→ 「**Agent の権限**」を **Yolo** にします。第1弾 3 章の逆の操作です。「インストール済み」の欄が `claude --dangerously-skip-permissions`、`codex --dangerously-bypass-approvals-and-sandbox` に変わります。
...
図の下段「ハンズオンでは手動」は、第1弾のときの設定です。続編では 1 層目が **Yolo** になります。
...
「詳細設定」を開くと、**名前が Issue のタイトルから自動で作られている** のが見えます（例: `formatdateja-2026-9-13`）。英字と数字だけが小文字で残り、日本語や記号は落ちます。ブランチ名は `<あなたのID>/formatdateja-2026-9-13` になります。ここは見るだけで、変えずに閉じて構いません。
...
プロンプトが出たら、画面を 3 か所確認します。

- 入力欄に Issue の URL が入っている
- その下に **「⚠ Pre-filled prompt · review before pressing Enter」** と出ている。「入力欄に文が入っているから、送る前に確認して」という Claude Code からの注意です
- 一番下の行に `bypass permissions on` と出ている。これが Yolo の印です

**まだ ↩ は押しません。** 5 章で 3 本まとめて送ります。
...
> ワークツリーの切り替えは **⌘J** でワークツリー名の一部（`coupon` など）を打つのが速いです（第1弾 8 章）。3 本並べて眺めたいときは ⌘J → ⇧↩（Shift+Enter）で分割ペインに開けます（第1弾 7-3）。
...
> Claude Code の入力欄に、灰色で「コミットして PR を作成してください」のような **提案文** が出ることがあります。Claude Code が次の一手を提案する機能です。
>
> 今回は Orca の Source Control から PR を作る練習なので、**この提案はそのまま ↩ しないでください**。入力欄をクリックして自分の文を打てば消えます。
...
これから押すボタンと、裏で起きる git の操作の対応を先に見ておきます。**ボタンの文言が、今どの段階にいるかを教えてくれます**（第1弾 11 章の再掲）。

![Source Control のボタンと git 操作の対応。ステージオール → Commit → ブランチを公開 → PR を作成の順に進み、PR #4 が OPEN になる](images/diagrams/03_source-control-flow.svg)

### 6-1. 差分を読む

サイドバー（または ⌘J）で **#1 のワークツリー**（formatDateJa）を選びます。右パネルの **Source Control**（3 番目のブランチアイコン）を開きます。右パネルが閉じていれば、右上のパネル開閉アイコンで開きます。「変更点 2」に `date.js` と `date.test.js` が並んでいます。
...
- [ ] **正しいか**（Issue の仕様どおりか）: Issue の表の 3 例（`2026年9月13日`、`2026年1月5日`、`2026年12月25日`）がテストに入っているか。ゼロ埋めしていないか
...
Source Control で「**＋ ステージオール**」を押すと、見出しが「ステージ済みの変更 2」になり、メッセージ欄が有効になります。

メッセージ欄右上の **AI アイコン** でコミットメッセージを生成します（10〜20 秒。英語で出ることもあります）。初回だけ「コミットメッセージの生成」ダイアログが出ることがあります。そのまま「Generate」を押してください（第1弾 11-2）。**生成された文を読んで、事実と違えば直してください**。
...
`Closes #N`（`Fixes #N`、`Resolves #N` でも同じ）は GitHub のキーワードです。**このコミットが `main` に入った時点で、Issue #N が自動的に閉じます**。PR の本文に書いても同じ効果があります。今回の検証では Orca が PR の本文を空で作ったので、コミットメッセージに書くのが確実です。

> バージョンによっては、「PR を作成」を押したときにタイトルと説明を確認するダイアログが出ます（第1弾 11-4）。出た場合は、説明欄にも `Closes #1` と書いておくと確実です。
...
コミットすると、ボタンが「**ブランチを公開**」（Publish Branch）に変わり、右上に `↑1` が出ます。押して push します。`↑1` の表示が消えれば push 完了です。

![コミット後。ボタンが「ブランチを公開」になり、コミット先のブランチに 2 ファイルが並ぶ](images/09_sc_publish_branch.png)

push が終わったら、パネル上部の「**PR を作成**」を押します。第1弾と同じく、確認ダイアログなしで PR ができます。右パネルが自動で **PR パネル**（4 番目のアイコン）に切り替わります。
...
#2（Codex）→ #3（Claude Code）の順に、6 章と同じ手順を繰り返します。この順に作ると PR の番号が #5、#6 になり、以降の本文と一致します。順番が違っても手順は同じで、番号だけ読み替えてください。
...
> 8〜11 章が本編の後半です。ここから **`main` が動き始めます**。
...
赤い ✗ になったら、チェック名をクリックすると失敗したジョブのログがその場で読めます。ワークツリーのカードにも赤いチップが付きます。PR パネルの「**Fix broken checks**」を押すと、失敗したチェック名とリンクをエージェントに渡して直させることができます（付録 B）。

### 8-2. マージする

緑の「**Create merge commit**」を押します。**確認ダイアログは出ず、数秒でマージされます。**

ボタンの文言は **リポジトリの既定のマージ方式** に従います（`Create merge commit` / `Squash and merge` / `Rebase and merge` のどれか）。複製直後のリポジトリは `Create merge commit` です。
...
ターミナル（リポジトリの中ならどこでも構いません）で確認します。
...
#2 のワークツリーを選び、8 章と同じ手順でマージします（チェックが緑 → Create merge commit）。#1 は `date.js` だけ、#2 は `price.js` だけを触っているので、**すんなり** 入ります。`gh issue view 2 --json state` が CLOSED になります。

### 9-2. #3 の PR を見る

**マージ直後に #3 のワークツリーに切り替えて、PR パネルの更新アイコンを押してください。** さっきまで緑だった #3 の PR が、次のように変わります。

- 緑のボタンが「**競合**」の表示に変わり、薄くなって押せない
- 「**競合によりこれがブロックされます PR**」「チェックとマージが完了する前に競合を解決してください。」という警告と、「**解決**」ボタン
- 「**4 commits 遅れ**（ベースコミット: …）」… `main` が 4 コミット先に進んだ、という意味。4 は参考値です（#1 と #2 が「エージェントのコミット 1 つ + マージコミット 1 つ」で 2 つずつ。エージェントのコミット数で変わります）
...
| 場所 | 開き方 | この章でやること |
|---|---|---|
| ブランクターミナル（中央のタブ） | タブバーの ＋ → 「ブランクターミナル」 | `git merge`、`npm test`、`git commit`（10-1、10-4、10-5） |
| Claude Code のターミナル（中央のタブ） | 5 章から動いているタブ | 競合の解消を頼む（10-3） |
| Conflict Review タブ（中央） | Source Control の「競合をレビューする」 | ぶつかった箇所を見る。解消後に「更新」で確認（10-2、10-4） |
| Source Control パネル（右、3 番目） | ブランチのアイコン | 競合ファイルの一覧、ステージ状況、「プッシュ」（10-1、10-4、10-5） |
| PR パネル（右、4 番目） | チェックリストのアイコン | 競合の警告が消えたか、チェック、「Create merge commit」（10-5） |

> **「解決」と「AIで解決する」ボタンについて**
>
> - PR パネルの「**解決**」と、次に出てくる Source Control の「**AIで解決する**」は、この作業をまとめてエージェントに任せる入口です
> - ただし **本書の検証（1.4.200）では、押すと新しいタブが開くだけで、エージェントが起動しませんでした**
> - そのため本書では、取り込みは自分で打ち（10-1）、解消はワークツリーで動いている Claude Code に直接頼みます（10-3）
> - ボタンが動く環境なら、押すと次のダイアログ（使う Agent とプロンプトの確認）が出て、同じことが自動で進みます（付録 B）。見かけたら「これのことか」と分かる程度に覚えておいてください
>
...
#3 のワークツリーで、エージェントのターミナルとは別に **ブランクターミナル** を開きます（中央のタブバーの **＋** → 「ブランクターミナル」）。

プロンプトが `applycoupon >` のように **ワークツリー名** になっていることを確かめてください（`pwd` が `~/orca/workspaces/orca-handson-2/…` なら正しい場所です）。`~/Documents/orca-handson-2` になっていたらプライマリです。そのタブを閉じ、サイドバーで #3 のワークツリーを選び直してから開いてください。
...
「**競合をレビューする**」を押すと、中央に **Conflict Review** タブが開きます。

- 左: ファイル一覧（未解決の印付き）
- 右: コンフリクトマーカーを色分けしたエディタ。「前の競合 / 次の競合」で移動できます

**直す前に、何と何がぶつかっているのかを自分の目で見ておいてください。**

![Conflict Review タブ。price.js の末尾で applyCoupon（自分側）と calcShipping（origin/main 側）がぶつかっている](images/16_conflict_review_tab.png)

上の画像では、`=======` の上が自分のブランチ（`applyCoupon`）、下が `origin/main` から来た `calcShipping` です。今回の正解は **「両方残す」** です。解消後は次の形になります。

- 定数ブロック: `SHIPPING_FEE` / `FREE_SHIPPING_THRESHOLD` と `COUPONS` の両方
- ファイル末尾: `calcShipping` と `applyCoupon` の両方
- テスト: 両方の `describe` と、`import` に両方の関数名
...
PR パネルの更新アイコンを押し、`test` が Successful になったら「**Create merge commit**」を押します。
...
次の図は第1弾の再掲です。`fix-tests-claude` を今回の 3 本に読み替えてください。今回は 3 本ともコミット済み・マージ済みなので、「未コミットの変更」も「未マージのコミット」もありません。

![ワークツリー削除で消えるもの・残るもの。フォルダと未コミットの変更とローカルブランチは消える。プライマリ、push 済みブランチ、PR は残る](images/diagrams/06_worktree-cleanup.svg)

今回は 3 本とも **マージ済み** なので、ローカルブランチは黙って消えます。第1弾 12 章で触れた「コミット済みで未マージのブランチを退避する」動き（第1弾では「Preserved branches」と書いたもの。「Review N Branches」のトーストが出ます）は、**未マージのコミットがあるときだけ** 起きます。
...
`rebase` は **自分のブランチの履歴を書き換える** ので、すでに push 済みのブランチには普通の `push` ができません。このとき Orca の Source Control には「**Force push with lease**」が **別のボタンとして** 出ます。

`--force-with-lease` は、「リモートが自分の知っている状態のままなら上書きする」という安全弁付きの force push です。Orca が普通のプッシュの代わりに黙って force push することはありません。
...
| 症状 | 見るところ・直し方 |
|---|---|
| `gh repo create` が `name already exists` と言う | 同名のリポジトリが自分のアカウントにある。第1弾の付録 B で別名として `orca-handson-2` を使った人はここで衝突する。`orca-handson-2b` など別名で作り、以後のリポジトリ名を読み替える |
...
| チェックが赤（失敗）になった | チェック名をクリックしてログを読む。PR パネルの「Fix broken checks」でエージェントに渡す。直して push すると再実行される |
| #2 をマージしても #3 が競合しなかった | エージェントの書き方によっては同じ場所に足さないことがある。その場合は #3 をそのままマージして構いません。10-1 の `git fetch origin && git merge origin/main` だけ打ってみると、競合なしで取り込めるか `Already up to date` が出ます。競合しない取り込みも一度見ておくと感覚がつかめます |
...
| ブランクターミナルのプロンプトが `~/Documents/orca-handson-2`（プライマリ）になっている | サイドバーで #3 のワークツリーを選んでから ＋ → 「ブランクターミナル」を開き直す。`pwd` が `~/orca/workspaces/orca-handson-2/…` なら正しい。プライマリで `git merge origin/main` しても競合は出ない |
